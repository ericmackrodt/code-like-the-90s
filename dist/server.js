...
function fetchProps() {
    return { listItems: db_1.getListItems(), menuItems: db_1.getMenuItems() };
}
...
const mime_types_1 = __webpack_require__(/*! ./mime-types */ "./src/mime-types.ts");
const cache_control_1 = __webpack_require__(/*! ./cache-control */ "./src/cache-control.ts");
const constants_1 = __webpack_require__(/*! ./constants */ "./src/constants.ts");
console.log('Server booting...');
const isProd = "development" === 'production';
console.log('Production optimization enabled? ', isProd);
const AppFactory = react_1.createFactory(app_1.default);
...
async function handler(req, res) {
    ...
    try {
        if (url === 'index.html') {
            res.setHeader('Content-Type', mime_types_1.lookup(url));
            res.setHeader('Cache-Control', cache_control_1.control(isProd, 1));
            res.write(`<!DOCTYPE html>
            <html>
            <head>
                <meta charset="utf-8" />
                <meta name="viewport" content="width=device-width, initial-scale=1.0">
                <link href="${constants_1.faviconUrl}" rel="icon" type="image/x-icon" />
                <title>React Example</title>
                <link rel="stylesheet" href="${constants_1.stylesUrl}" />
            </head>
            <body>
            <div id="${constants_1.containerId}">`);
            const stream = server_1.renderToNodeStream(AppFactory(props_1.fetchProps()));
        ...
        else if (url === constants_1.propsUrl) {
            res.setHeader('Content-Type', mime_types_1.lookup(url));
            res.setHeader('Cache-Control', cache_control_1.control(isProd, 0));
            res.end(JSON.stringify(props_1.fetchProps()));
        }
        else if (url === constants_1.stylesUrl) {
            res.setHeader('Content-Type', mime_types_1.lookup(url));
            res.setHeader('Cache-Control', cache_control_1.control(isProd, 7));
            const file = `./src/${url}`;
            fs_1.createReadStream(file).pipe(res);
        }
        else if (url === constants_1.browserUrl || url === constants_1.browserMapUrl) {
            res.setHeader('Content-Type', mime_types_1.lookup(url));
            res.setHeader('Cache-Control', cache_control_1.control(isProd, 7));
            const file = `./dist${url}`;
            fs_1.createReadStream(file).pipe(res);
